...
export class App {
  ...
  onAffiliateToggle(s: SystemSettings, value: string) {
    if (!s.affiliateCommissionConfig) {
      s.affiliateCommissionConfig = {
    ...
    s.affiliateCommissionConfig.isAffiliateSystemEnabled = value === 'true';
  }

  onAffiliateGeneralRateChange(s: SystemSettings, value: string) {
    if (!s.affiliateCommissionConfig) {
      s.affiliateCommissionConfig = {
        generalCommissionRate: 10,
        minimumPayoutAmount: 100,
    ...
    s.affiliateCommissionConfig.generalCommissionRate = Number(value);
  }

  onAffiliateMinPayoutChange(s: SystemSettings, value: string) {
    if (!s.affiliateCommissionConfig) {
      s.affiliateCommissionConfig = {
        generalCommissionRate: 10,
        minimumPayoutAmount: 100,
    ...
    s.affiliateCommissionConfig.minimumPayoutAmount = Number(value);
  }

  getServiceCommissionRate(s: SystemSettings, serviceId: string): number | string {
    const rate = s.affiliateCommissionConfig?.serviceCommissionRates?.[serviceId];
    return rate !== undefined && rate !== null ? rate : '';
  }

  onServiceCommissionRateChange(s: SystemSettings, serviceId: string, value: string) {
    if (!s.affiliateCommissionConfig) {
      s.affiliateCommissionConfig = {
        generalCommissionRate: 10,
        minimumPayoutAmount: 100,
    ...
    if (!s.affiliateCommissionConfig.serviceCommissionRates) {
      s.affiliateCommissionConfig.serviceCommissionRates = {};
    }
    if (value === '') {
      delete s.affiliateCommissionConfig.serviceCommissionRates[serviceId];
    } else {
      s.affiliateCommissionConfig.serviceCommissionRates[serviceId] = Number(value);
    }
  }
